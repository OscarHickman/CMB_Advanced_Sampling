...
**⚠ This layout carries 2L real dof per multipole, not 2L+1, and that has two consequences you must know before touching any spectrum block or writing any calibration test.** `alm_utils.py::splittosingularalm` writes `complex(real, 0)` when `m == 0 or m == 1`, so `Im(a_{L,1})` is forced to zero at every multipole.

- **Statistical:** the Gaussian prior normalisation is `C^{-k_L/2}` with `k_L = 2L`, so the exact conditional against a prior `InvGamma(a0,b0)` is `InvGamma(k_L/2 + a0, b0 + S_L/2)`. Use `invgamma_shape_for_spectrum`, never a hardcoded `L - 0.5`.
- **Physical, and still OPEN:** the model therefore *cannot represent a general sky* — one real dof per ℓ is missing, ~20% of the modes at ℓ=2 falling to 0.8% at ℓ=63. This is the largest known open defect (`ROADMAP.md`); restoring it would also make `k_L = 2L+1`. Note ~78 sites across ~28 files hardcode `n_imag = (lmax-2)(lmax-1)/2`, and changing it invalidates every existing checkpoint.

### `tests/` vs `scripts/`

`tests/` holds fast, deterministic pytest coverage at small lmax (`test_lensing.py`, `test_samplers.py`, `test_sht_ducc.py`, etc.) — run these for any code change. `scripts/` is production/validation infrastructure, not unit tests: `gate_*.py` are one-off go/no-go checks before scaling a sampler configuration to lmax=300 (results recorded in `achievements.md`, not re-run routinely); `debug_*.py` are investigation scripts from specific past bugs (see `achievements.md`'s "Real bugs" list for which); `validate_coverage_rank_nulls.py` calibrates the coverage-ensemble rank statistics against what a *correct* sampler produces — **run it before reading any spectrum-row FLAG from `aggregate_coverage_ranks.py` as bias**, because that statistic ranks the truth against its own conditional's mode and is non-uniform by construction; `submit_*.slurm`/`*.slurm` are the SLURM wrappers for the matching `.py` script, following a consistent pattern (`-A durham -p dine2`, per-job `$TMPDIR`, `PYTHONPATH` set to `diffcmb/`). When adding a new production-scale validation, mirror an existing gate/smoke script's structure rather than inventing a new one.
