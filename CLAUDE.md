...
- **Physical, and still OPEN:** the model therefore *cannot represent a general sky* — one real dof per ℓ is missing, ~20% of the modes at ℓ=2 falling to 0.8% at ℓ=63. This is the largest known open defect; restoring it would also make `k_L = 2L+1` and thus vindicate the original `L - 0.5` shape (so don't read the git history as a reverted fix). **Scoped, not attempted — read `docs/notes/restore_missing_alm_dof_scoping.md` before touching the packing.** Measured blast radius: **276 sites across 37 files**, the literal `n_imag = (lmax-2)*(lmax-1)//2` appearing **75 times** verbatim, and 224 saved `.npz` / 12 GB of which **107 are checkpoints** — and since `run_gibbs_chain` resumes automatically from a checkpoint, a layout change silently resumes in-flight campaigns into a wrong-length vector.
