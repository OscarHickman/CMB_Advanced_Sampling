...
- [ ] **MCLMC spike (φ block only) — active workstream.** Scope: port only the φ|alm,C_ℓ HMC block (Block 3) to MCLMC, validate against the existing dense small-scale reference used for the original HMC φ-block validation, hard-abort if it doesn't beat the current HMC configuration on ESS/wall-clock. Do not touch Blocks 1/2/4.
  ...
  - **IN FLIGHT (check this first): job 11708844, `gate_phi_mclmc_vs_hmc_L128`, submitted 2026-08-08 ~01:2x, dine2/durham, 12h walltime.** Scales the winning small-lmax config to the actual pilot scale/problem bin: `lmax=128, nside=64, n_burnin=100, n_samples=200, phi_n_lfs=240` (HMC baseline, matches the recorded best HMC config exactly) vs MCLMC `n_steps=30, step_size=0.1, L∈{200,350}` (L scaled ~6.6x by sqrt(d_128/d_20), tried at two nearby values since that scaling is a rule-of-thumb). Ranks by ESS/s at l≈77 (falls inside the problem bin l=[60,100)). **Next AI agent: check `squeue -u dc-hick2` — if finished, read** `logs/gate_phi_mclmc_vs_hmc_L128_11708844.out` **directly (no re-run needed, script already saved `results/analysis/phi_mclmc_vs_hmc_lmax128_pilot.npz`).** Decision:
    - MCLMC clearly beats HMC ESS/s at l≈77 (and doesn't badly regress at low l) → GO: make `phi_sampler='mclmc'` the production Block 3 path (update the `run_gibbs_chain` call in `scripts/pilot_coverage_equilibration.py`/checkpoint-resume tooling), re-run the lmax≈128 pilot equilibration check, record the win in `achievements.md`.
    - AMBIGUOUS/NO-GO at l≈77 → close the spike: revert to `phi_sampler='hmc'` (already the default, no code change needed) as the only production path, record the finding (the promising-but-unresolved small-lmax lead, and why it didn't transfer at l≈77 if that's what happened) in `achievements.md`'s "Closed-out sampler routes", and fall back to whatever the roadmap's next-priority item is (manuscript, coverage ensemble prep once a sampler is settled, etc.) — `phi_sampler='mclmc'` code stays in the repo either way (tested, documented, opt-in), just not used in production if it doesn't win.
  - Do not spend further tuning cycles beyond this one pilot-scale run without checking with the user first — see [[feedback_no_login_node_jobs]] and the session's repeated cost warnings; this spike already went through 4 dine2 jobs (untuned lmax=20, step/L grid, n_steps grid, lmax=128 pilot) and one more ambiguous result should be reported and decided together with the user, not auto-escalated to a 5th round.
