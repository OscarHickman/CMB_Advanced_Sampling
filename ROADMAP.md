...
- **v2 MCLMC pilot (`pilot_coverage_lmax128_mclmc_v2`) — job 11744132 COMPLETED 2026-08-13, re-analyzed 2026-08-14: NO-GO.** After the infra saga (5 failed/cancelled submissions on an `/cosma/apps` disk-quota race, fixed by moving the checkpoint to `/cosma8/data/dp004/dc-hick2/diffcmb_results_archive/` — full timeline in `achievements.md`), the run completed its full 3700-sweep window at the tuning grid's recommended non-oscillating config (step_size=0.005, L=100, n_steps=30). **Result: worst lag-1 autocorrelation is 1.000 and worst drift is 6.55σ across all 5 l-bins — none cross the |r_k|<0.2 gate even by lag 200, and this is *worse*, not better, than the untuned run (job 11710475, lag-1≈0.975-0.99).** This is exactly the outcome the v2 submission script's own pre-registered comment predicted and pre-committed to escalating on. **Per the `Sampler-lever decision` rule above, this is now trigger 3(a)/(b): MCLMC has failed the long-run equilibration gate twice (untuned and tuned) despite winning the ESS/wall-clock-second side-by-side both times (job 11708844) — reads as a sampler-geometry problem, not a tuning problem.** No further tuning grid has been launched. **This needs an explicit user decision**: options per the roadmap's own framework are (a) accept the schedule cost and try a further reparametrisation/geometry fix for MCLMC, (b) fall back to accepting HMC's best result (also NO-GO at phi_n_lfs=240, so this likely means lowering lmax further for the coverage test rather than fixing lmax=128), or (c) something else — this needs your call, not a unilateral next step.
- **No jobs currently queued or running for this repo** (checked `squeue -u dc-hick2`; only unrelated Galform/L800/assembly_bias/bt_/fd_ jobs are active). Nothing to resubmit right now.
