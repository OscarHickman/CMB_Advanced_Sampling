...
6. **Recommended next step, needs sign-off:** re-run job 11836793's exact configuration (lmax=64, `phi_mass_matrix='block'`, Block 4 ON) post-fix — one ~15h chain, not an ensemble. It is simultaneously the paper's production configuration, the only untested-post-fix mass matrix, and the one mechanism aimed at exactly the cross-L/band-edge coupling that `[60,64)` exhibits. Score it with `rescore_phi_equilibration.py`, not the lag-1 gate. If it clears, the lmax=64 coverage ensemble launches on it and delivers both critical-path items at once. Per the standing no-unilateral-tuning rule, not launched.
