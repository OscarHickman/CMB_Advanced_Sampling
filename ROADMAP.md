...
6. **Actioned 2026-08-26: job 11874976 launched** (`submit_pilot_coverage_lmax64_block_cl4_ordfix.slurm`) — re-runs job 11836793's exact configuration (lmax=64, `phi_mass_matrix='block'`, Block 4 ON) post-fix. It is simultaneously the paper's production configuration, the only untested-post-fix mass matrix, and the one mechanism aimed at exactly the cross-L/band-edge coupling that `[60,64)` exhibits in job 11849969. ~11h expected (job 11836793's own wall-clock). **Score it with `scripts/rescore_phi_equilibration.py` (τ_int/ESS/split-R̂), not the raw lag-1<0.9 gate** — that gate was shown to be a measurement artifact on 11849969 (`achievements.md`). If it clears, the lmax=64 coverage ensemble launches on this exact configuration and delivers both critical-path items (exactness evidence + headline joint-posterior figure) at once. Check `sacct -j 11874976` / `logs/pilot_coverage_lmax64_block_cl4_ordfix_11874976.out` next session.
