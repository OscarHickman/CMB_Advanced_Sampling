...
\placeholder{$N = 17856$}). We report the accompanying control rather than only
the statistic: a deliberately lag-shifted pairing is rejected only when $\phi$
mixes fast enough for $S_L(\phi_i) \ne S_L(\phi_{i+1})$ to bite
(\placeholder{$p = 0.006$} in an earlier configuration), and in the
proper-prior chains reported here it is \emph{not} rejected
(\placeholder{$p = 0.18$}), so this particular pass carries no weight on its
own. It is included for completeness, and the calibration evidence below rests
on the independent generative ranks instead.
