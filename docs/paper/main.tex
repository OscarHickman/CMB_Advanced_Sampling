...
\begin{equation*}
  \bar u_{\phi} = \placeholder{0.453}\ (p = \placeholder{0.17}), \qquad
  \bar u_{\alm} = \placeholder{0.537}\ (p = \placeholder{0.33}),
\end{equation*}
both consistent with uniformity, against $\bar u_\phi = \placeholder{0.367}$
($p = \placeholder{0.004}$) for the same configuration with $\clpp$ resampled
under the flat improper prior. \todo{Add rank histograms.}
...
produces the joint posterior of Sec.~\ref{sec:joint-posterior}; this is the
configuration we recommend for quoting $\clpp$ constraints.
\todo{Report the proper-prior ensemble once re-run against the corrected shape parameter.}
